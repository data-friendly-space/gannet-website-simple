...
export default function Testimonials() {
  ...
  return (
    <section className={`section-padding-small ${bgClass} relative`}>
      {/* Background elements */}
      ...
      <div className="max-w-7xl mx-auto container-padding relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((testimonial, index) => (
            <motion.div
              key={testimonial.id}
              className={`${cardBg} border ${cardBorder} rounded-xl shadow-lg card-padding flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1`}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              {/* Quote icon */}
              <div className={`mb-6 ${isDark ? 'text-gannetBlue/40' : 'text-gannetBlue/20'}`}>
              ...
              {/* Testimonial text */}
              <p className={`${textClass} text-body mb-8 flex-grow`}>
                &ldquo;{testimonial.text}&rdquo;
              </p>
              
              {/* Author info */}
              <div className="flex items-center">
                <div className={`flex items-center justify-center h-14 w-14 rounded-full border-2 border-gannetGreen ${getColorFromName(testimonial.name)} text-white font-bold text-lg`}>
                  {getInitials(testimonial.name)}
                </div>
                <div className="ml-4">
                  <h4 className={`font-semibold text-body-small ${nameClass}`}>{testimonial.name}</h4>
                  <p className={`${roleClass} text-caption`}>
